...
if Options.SplitMech==1 && Options.CustomMRIPost==1
    ...
    for i=1:nCond
        ...
        for j=1:length(freqOut)
            ...
            sol=Dynamic(:,count);
            % Dissipated Power
            [OutPower4K(count),OutPower77K(count),OutPowerOVC(count)]=PowerCalculationStaggeredMRI(Mesh,Unknown,Basis,Quadrature,sol,ProblemData,freq,UnknownStatic,solStatic,condOVC,cond77K,cond4K);
