function [unknown]=nounk_static(mesh,ProblemData)
...
for i=1:nelem
    for j=1:4
        if  cond(i,j)==2 || cond(i,j)==7 || cond(i,j)==8 || cond(i,j)==10 
...
% treat 3 Neumann
for i = 1:nside
    if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8 && bcedge(i)~=10  && bcedge(i)~=68 && bcedge(i)~=67 && bcedge(i)~=610 && bcedge(i)~=128 && bcedge(i)~=137 && bcedge(i)~=138 
        nunk = nunk +1;
        unkz(i) = nunk;
    elseif bcedge(i) ==2 || bcedge(i)==7 || bcedge(i)==8 ||bcedge(i)==10 ||bcedge(i)==68 ||bcedge(i)==67 ||bcedge(i)==610 ||bcedge(i)==128 ||bcedge(i)==137 ||bcedge(i)==138 
        npec = npec+1;
        unkz(i) = -1*npec;
    end
end
...
if order >=1
    for i=1:nside
        %        if help2(i)==0
        if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8 && bcedge(i)~=10 && bcedge(i)~=68 && bcedge(i)~=67 && bcedge(i)~=610 && bcedge(i)~=128 && bcedge(i)~=137 && bcedge(i)~=138  && help2(i)==0
            for j = 1:order
                nunk = nunk+1;
                unksid(i,j) = nunk;
            end
...
if order >=2
    ...
    for i =1:nface
        ...
        elseif help1(i)==2 || help1(i)==7 || help1(i)==8 || help1(i)==10 
            k=0;
            for j1=0:order-2
                for j2=0:order-2
                    if (j1+j2)<=(order-2)
