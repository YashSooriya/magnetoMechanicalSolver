...
function [unknown]=nounk(mesh,ProblemData)
...
for i=1:nelem
    for j=1:4
        if  cond(i,j)==2 || cond(i,j)==7 || cond(i,j)==8 || cond(i,j)==13
...
% treat 3 Neumann
for i = 1:nside
    if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8 && bcedge(i)~=68 && bcedge(i)~=128 && bcedge(i)~=13 && bcedge(i)~=137 && bcedge(i)~=127 && bcedge(i)~=138 && bcedge(i)~=613 && bcedge(i)~=67
        nunk = nunk +1;
        unkz(i) = nunk;
    elseif bcedge(i) ==2 || bcedge(i)==7 || bcedge(i)==8  || bcedge(i)==68 || bcedge(i)==128 || bcedge(i)==13 || bcedge(i)==137 || bcedge(i)==127 || bcedge(i)==138 || bcedge(i)==613 || bcedge(i)==67
        npec = npec+1;
        unkz(i) = -1*npec;
    end
end
...
if order >=1
    for i=1:nside
        %        if help2(i)==0
        if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8  && bcedge(i)~=68 && bcedge(i)~=128 && bcedge(i)~=13 && bcedge(i)~=137 && bcedge(i)~=127 && bcedge(i)~=138 && bcedge(i)~=613 && bcedge(i)~=67 && help2(i)==0
            for j = 1:order
                nunk = nunk+1;
                unksid(i,j) = nunk;
            end
...
if order >=2
    ...
    for i =1:nface
        ...
        elseif help1(i)==2 || help1(i)==7 || help1(i)==8 || help1(i)==13 
            k=0;
            for j1=0:order-2
                for j2=0:order-2
                    if (j1+j2)<=(order-2)
...
for i=1:nelem
    for j=1:4
        if  cond(i,j)==6 || cond(i,j)==7 || cond(i,j)==8 || cond(i,j)==10 || cond(i,j)==12 || cond(i,j)== 13 
...
%=======================================================================================
% Number the DOF for each mechanical subdomain
%=======================================================================================
for nBody=1:NmechBodies
    nunkMechTemp=nunkMech;
for i=1:nNodes
    [row,~]=find(intma==i);
    if any(ismember(mat(row),matMech(nBody,:)))
        FlagMechVolume(row)=1;
        % ii=mapG2L_n(i);
        %if ii~=0
        if bcvertex(i)~=6 && bcvertex(i)~=7 && bcvertex(i)~=8 && bcvertex(i)~=10 && bcvertex(i)~=12 && bcvertex(i)~=68 && bcvertex(i)~=67 && bcvertex(i)~=610 && bcvertex(i)~=128 && bcvertex(i)~=13 && bcvertex(i)~=137 && bcvertex(i)~=138 && bcvertex(i)~=613 
            for k=1:3
                nunkMech=nunkMech+1;
                nunkt=nunkt+1;
        ...
        elseif bcvertex(i)==6 || bcvertex(i)==68 || bcvertex(i)==610 || bcvertex(i)==613 || bcvertex(i)==67
            for k=1:3
                npecMech = npecMech+1;
                npec=npec+1;
        ...
        elseif bcvertex(i)==10
            ...
            npecMech=npecMech+1;
            npec=npec+1;
            unkvertexMech(i,2)=-npecMech;
            unkvertexSystem(i,2)=-npec;
            npecMech=npecMech+1;
            npec=npec+1;
            unkvertexMech(i,3)=-npecMech;
            unkvertexSystem(i,3)=-npec;          
        elseif bcvertex(i)==12 
            nunkMech=nunkMech+1;
            nunkt=nunkt+1;
            unkvertexMech(i,1)=nunkMech;
            unkvertexSystem(i,1)=nunkt;
            ...
            npecMech=npecMech+1;
            npec=npec+1;
            unkvertexMech(i,3)=-1*npecMech;
            unkvertexSystem(i,3)=-1*npec; 
        elseif bcvertex(i)==128
            nunkMech=nunkMech+1;
            nunkt=nunkt+1;
            unkvertexMech(i,1)=nunkMech;
            unkvertexSystem(i,1)=nunkt;
            ...
            unkvertexMech(i,3)=-1*npecMech;
            unkvertexSystem(i,3)=-1*npec; 
       elseif bcvertex(i)==13 || bcvertex(i)==137 || bcvertex(i)==138
            npecMech=npecMech+1;
            npec=npec+1;
            unkvertexMech(i,1)=-npecMech;
            unkvertexSystem(i,1)=-npec;
            npecMech=npecMech+1;
            npec=npec+1;
            unkvertexMech(i,2)=-1*npecMech;
            unkvertexSystem(i,2)=-1*npec; 
    ...
    for i=1:nside
        ...
        if any(ismember(mat(row),matMech(nBody,:)))
            ...
            for j=0:orderH1-2
                ...
                if bcedge(i)~=6 && bcedge(i)~=7 && bcedge(i)~=8 && bcedge(i)~=10 && bcedge(i)~=12 && bcedge(i)~=68 && bcedge(i)~=67 && bcedge(i)~=610 && bcedge(i)~=128 && bcedge(i)~=13 && bcedge(i)~=137 && bcedge(i)~=138 && bcedge(i)~=613
                    for k=1:3
                        nunkMech=nunkMech+1;
                        nunkt=nunkt+1;
                ...
                elseif bcedge(i)==6 || bcedge(i)==68 || bcedge(i)==610 || bcedge(i)==613 || bcedge(i)==67
                    for k=1:3
                        npecMech=npecMech+1;
                        npec=npec+1;
                ...
                elseif bcedge(i)==10
                    ...
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkedgesMech(i,j+1,2)=-npecMech;
                    unkedgesSystem(i,j+1,2)=-npec;
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkedgesMech(i,j+1,3)=-1*npecMech;
                    unkedgesSystem(i,j+1,3)=-1*npec;
                elseif bcedge(i)==12
                    nunkMech=nunkMech+1;
                    nunkt=nunkt+1;
                    unkedgesMech(i,j+1,1)=nunkMech;
               ...
               elseif bcedge(i)==128
                    ...
                    unkedgesMech(i,j+1,3)=-1*npecMech;
                    unkedgesSystem(i,j+1,3)=-1*npec;
                elseif bcedge(i)==13 || bcedge(i)==137 || bcedge(i)==138
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkedgesMech(i,j+1,1)=-1*npecMech;
                    unkedgesSystem(i,j+1,1)=-1*npec;
        ...
        if any(ismember(mat(row),matMech(nBody,:)))
            ...
            for j=1:((orderH1-1)^2-(orderH1-1))/2
                ...
                elseif helpF(i)==10
                    ...
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkfacesMech(i,j,2)=-npecMech;
                    unkfacesSystem(i,j,2)=-npec;
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkfacesMech(i,j,3)=-1*npecMech;
                    unkfacesSystem(i,j,3)=-1*npec;
                elseif helpF(i)==12
                    nunkMech=nunkMech+1;
                    nunkt=nunkt+1;
                    unkfacesMech(i,j,1)=nunkMech;
               ...
               elseif helpF(i)==13 
                    ...
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkfacesMech(i,j,1)=-npecMech;
                    unkfacesSystem(i,j,1)=-npec;
                    npecMech=npecMech+1;
                    npec=npec+1;
                    unkfacesMech(i,j,2)=-1*npecMech;
                    unkfacesSystem(i,j,2)=-1*npec;
...
if orderH1>=4
    for i = 1:nelem
        if ismember(mat(i),matMech(nBody,:))
            FlagMechVolume(i)=1;
            k=0;
            for ii=0:orderH1-4
                for jj=0:orderH1-4
...
nunkC(nBody)=nunkMech-nunkMechTemp;
end
