function [L2err_mech,Energy_error_mech,SNS_error_mech] =ErrorStatic(mesh,unknown,Basis,Quadrature,gorder,order,orderH1,esizet,esizeH1,...
    sol,probdata,probstatic,known_mech)
...
mapL2G_ed=mesh.mech.mapL2G_ed;
mapL2G_fa=mesh.mech.mapL2G_fa;

% Extract relevant data from unknown structure
unkvertexSystem=unknown.system.unkvertexSystem;
unkedgesSystem=unknown.system.unkedgesSystem;
unkfacesSystem=unknown.system.unkedgesSystem;
...
for i=1:nelem
   aaa=mapL2G_e(i);
    ...
    % work out numbering of basis functions
    bhelp=rowfun(unknown,order,orderH1,mesh,...
        aaa,esizet,esizeH1,subFlag(aaa));
    
    %         % extract local solution for this element
    lsol=zeros(3*esizeH1,1);
    for ii=1+esizet:esizet+3*esizeH1
        row=bhelp(ii);
        if row>0
            lsol(ii-esizet,1)=sol(row,1);
        end
    end
...
Energy_error_mech=sqrt(EnergyError/ExactNormSum);
SNS_error_mech=sqrt(SNSerror/exactNormSNSsum);
