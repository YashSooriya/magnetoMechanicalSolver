function myplot3(mesh,unknown,probdata,Solution,problem,probstatic,freq)
       ...
       if gorder>1
        ...
        if flag==0
            ...
            if eltype(cores(i,2))==1
                ...
                H1basis(i,1:esizeH1)=th1(1:esizeH1,1)';
            else
            ...
            H1bas3D(1,1:3:end)=H1basis;
            H1bas3D(2,2:3:end)=H1basis;
            H1bas3D(3,3:3:end)=H1basis;

                lunkv=unknown.EM.unknowns(:,cores(i,2));
                lunkvM=unknown.Mech.unknownsD(:,cores(i,2));
            
            %     Magnetic Field
            he=zeros(esizet,1);
            he(lunkv>0) = sol(lunkv(lunkv>0),1);
            dispCoeff=zeros(3*esizeH1,1);
            dispCoeff(lunkvM>0)=sol(lunkvM(lunkvM>0),1);
            
            htil= real(phmxyz'*he/probdata.matr.mu(mat(cores(i,2)))/probdata.matr.muz);
            displ=H1bas3D*dispCoeff;
            displacement(fnd,:,:)=displ;
            outh(fnd,:,:)=htil;
...
mu=probdata.matr.mu;
    ...
    for i = 1:N
        ...
        [curleddy,hf,H0]=fun(xp,yp,zp,arg,probstatic,omega);
        end
        ...
        for j=1:nrhs
            ...
            dispPlot(i,j)=norm(displacement(i,:,j));
   ...
   % Saving variables to plot for different p for TEAM 7
        ...
        order=order+1;
         fileName=sprintf('B_AC_50Hz_p%d',order);
        save(fileName,'zcoord','bploz');
        ...
        order=order+1;
         fileName=sprintf('U_50Hz_p%d',order);
        save(fileName,'zcoord','dispPlot');
