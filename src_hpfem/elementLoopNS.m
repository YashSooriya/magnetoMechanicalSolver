function [mass,damp,damppre,dampReg,damppreReg,stiff,Resid,Kd,Cd,CdReg,Md,nmst]=elementLoop(Static,Mesh,Basis,Quadrature,Unknown,UnknownStatic,ProblemData,X,LeviCivita,coupling,freqSweep)
%=========================================================================
...
Kdval              = zeros((ndirEM+ndirM)^2,1);
Cdval              = zeros((ndirEM+ndirM)^2,1);
CdvalReg              = zeros((ndirEM+ndirM)^2,1);
Mdval              = zeros((ndirEM+ndirM)^2,1);
Idir             = zeros((ndirEM+ndirM)^2,1);
Jdir              = zeros((ndirEM+ndirM)^2,1);
...
for i=1:nelem
    ...
    dAdt(lunkvEM>0)=X(lunkvEM(lunkvEM>0),2);
    dAdt2(lunkvEM>0)=X(lunkvEM(lunkvEM>0),3);

    % Extract Mech solution (keep dirichlet in case storage issue)
    u(lunkvM>0)=X(lunkvM(lunkvM>0),1);
    dudt(lunkvM>0)=X(lunkvM(lunkvM>0),2);
    dudt2(lunkvM>0)=X(lunkvM(lunkvM>0),3);    
    ...
    %---------------------------------------------------------------------
    [M,C,K,Res,StressIntegral]=GaussInteg(Basis,Quadrature,ProblemData,...
        flag,mue,Adc,Xa,Xu,mycoord,eltype,probstatic,material,subFlag(i),...
        i,lec,lfc,xy,cond,LeviCivita,coupling,StressIntegral,FlagVolume);
    ...
    if subFlag(i)==2                                   % Conducting region
        if probstatic==0                            % Dynamic problem
            ...
            kappalowReg=0;
        elseif probstatic==1                        % Static problem
            kappalow=0; 
            kappalowReg=regTerm;
        end
    else   
