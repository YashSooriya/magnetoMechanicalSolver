function [OutVectorPotential] = NormACalculationStaggered(Mesh,Unknown,Basis,Quadrature,...
    sol,ProblemData,freq,UnknownStatic,solStatic,CondFactorBodies)
...
OutPower=zeros(1,NmechBodies);
OutVectorPotential=zeros(1,NmechBodies);
        ...
        if flag==0
            ...
            for pp=1:nip
                ...
                for nBody=1:NmechBodies
                    if any(material==matMech(nBody,:))
                        OutVectorPotential(nBody)=OutVectorPotential(nBody)+abs((e'*e))*intw(pp)*det;
                    end
                end
...
% magnitude 
OutVectorPotential = sqrt(OutVectorPotential)
disp(['The Output Norm of A is: ', num2str(OutVectorPotential(1))]);