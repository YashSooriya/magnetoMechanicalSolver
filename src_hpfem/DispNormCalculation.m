function [DispNorm] =DispNormCalculation(mesh,unknown,Basis,Quadrature,sol,probdata)
...
NmechBodies=probdata.NmechBodies;
matMech=probdata.matMech;

DispNorm=zeros(1,NmechBodies);
        ...
        for pp=1:nip
            ...
            for nBody=1:NmechBodies
                if any(material==matMech(nBody,:))
                    DispNorm(nBody)=DispNorm(nBody)+intw(pp)*det*abs(u_D'*u_D);
                end
            end
...
display(['The Displacement norm is = ',num2str(DispNorm(1))])
