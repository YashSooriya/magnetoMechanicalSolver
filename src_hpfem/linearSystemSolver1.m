function [U,CondNum,unknown]=linearSystemSolver1(Static,StaticCurrent,mesh,Basis,Quadrature,unknown,ProblemData,X,w0,w1,w2,Options)
...
if Assemble==1
  [M,Ccond,Ccondpre,CReg,CpreReg,K,Res,~,~,~,~,nmst]=elementLoop(Static,StaticCurrent,mesh,Basis,Quadrature,unknown,unknown,unknown,ProblemData,X,coupling,freqSweep,SourceMapping);
    save(['data/',fileName],'M','Ccond','Ccondpre','CReg','CpreReg','K','Res','nmst','-v7.3');
else
    load(['data/',fileName],'M','Ccond','Ccondpre','CReg','CpreReg','K','Res','nmst');
end
...
C=C+Ccond{nBody};
Cpre=Cpre+Ccondpre{nBody};
end
TSM = w2*M+w1*C +CReg+ w0*K;
TSMpre = w2*M+w1*Cpre+CpreReg+w0*K;
