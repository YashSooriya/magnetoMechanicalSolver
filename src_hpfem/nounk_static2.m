function [unknown]=nounk_static2(mesh,ProblemData)
...
% treat 3 Neumann
for i = 1:nside
    if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8  && bcedge(i)~=13 && bcedge(i)~=68 && bcedge(i)~=67 && bcedge(i)~=613  && bcedge(i)~=128 && bcedge(i)~=137 && bcedge(i)~=138 
        nunk = nunk +1;
        unkz(i) = nunk;
    elseif bcedge(i) ==2 || bcedge(i)==7 || bcedge(i)==8 || bcedge(i)==13 ||bcedge(i)==68 ||bcedge(i)==67 ||bcedge(i)==613  ||bcedge(i)==128 ||bcedge(i)==137 ||bcedge(i)==138
        npec = npec+1;
        unkz(i) = -1*npec;
    end
end
...
if order >=1
    for i=1:nside
        %        if help2(i)==0
        if  bcedge(i)~=2 && bcedge(i)~=7 && bcedge(i)~=8 && bcedge(i)~=13 && bcedge(i)~=68 && bcedge(i)~=67 && bcedge(i)~=613 && bcedge(i)~=128 && bcedge(i)~=137 && bcedge(i)~=138  && help2(i)==0
            for j = 1:order
                nunk = nunk+1;
                unksid(i,j) = nunk;
            end 
