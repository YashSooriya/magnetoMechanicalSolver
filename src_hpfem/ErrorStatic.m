function [L2err_mech,Energy_error_mech,SNS_error_mech] =ErrorStatic(mesh,unknown,Basis,Quadrature,...
    sol,probdata,probstatic,freq)
    ...
    if matc(i)==1
        ...
        if flag==0
            ...
            for pp=1:nip
                ...
                % use stored functions
                ph2(1:esizet,1:3)=(ecphx(pp,1:esizet)'*(asxi(1:3)))+...
                ...
                [curlexact,dum1,dum]=fun(x,y,z,arg,probstatic,omega);

                volume=volume+intw(pp)*det;
                for ii=1:3
                    ...
                    Berror=Berror+intw(pp)*det*abs(curle(ii)-curlexact(ii))^2;
                    Berrord=Berrord+intw(pp)*det*abs(curlexact(ii))^2;
...
Berror=sqrt(Berror/Berrord);
display(['The hcurl error is = ',num2str(hcurlerr)])
display(['The L2 error of B is = ',num2str(Berror)])

save('ErrorFluxB','Berror');
    ...
    if flag==0
        ...
        for pp=1:nip
            ...
            for ii=1:3
                ...
                DispNorm=DispNorm+intw(pp)*det*u_D(ii)*u_D(ii);
