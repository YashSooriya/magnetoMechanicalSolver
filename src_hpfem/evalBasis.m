 function [Basis]= evalBasis(Quadrature,ProblemData)
...
gphQuadx=zeros(nip,gesizet);
gphQuady=zeros(nip,gesizet);
gphQuadz=zeros(nip,gesizet);
...
for i=1:nip
    ...
    % Evaluate gradients of H1 basis functions for geometry order
    ...
    % Evaluate Basis functions for quadratic geometry jacobian
    gphQuad=BasisQuadraticGeom(intxi(i),inteta(i),intzeta(i));
    gphQuadx(i,1:10)=gphQuad(1:10,1)';
    gphQuady(i,1:10)=gphQuad(1:10,2)';
    gphQuadz(i,1:10)=gphQuad(1:10,3)';
...
Basis.gphQuadx=gphQuadx;
Basis.gphQuady=gphQuady;
Basis.gphQuadz=gphQuadz;
...
endpts(1)=-1;
endpts(2)=1;
[~,xie,~]=gaussq1(kind,nipe,0,0,kpts,endpts);
    ...
    for j=1:6
        ...
        for p=1:nipe
            ...
            elseif j==5
                ...
                % Evaluate Basis functions for quadratic geometry jacobian
    gphQuade=BasisQuadraticGeom(xi,eta,zeta);
    gphQuadex(p,1:10)=gphQuade(1:10,1)';
    gphQuadey(p,1:10)=gphQuade(1:10,2)';
    gphQuadez(p,1:10)=gphQuade(1:10,3)';
...
ptVector=zeros(3,3,4);

% Compute normal on reference element
    nmf=zeros(4,3);
    ...
    for j=1:4
        ...
        for pp=1:nipf
            ...
            if eltype==1
                ...
                % Evaluate Basis functions for quadratic geometry jacobian
                gphQuadf=BasisQuadraticGeom(xi,eta,zeta);
                gphQuadfx(pp,1:10)=gphQuadf(1:10,1)';
                gphQuadfy(pp,1:10)=gphQuadf(1:10,2)';
                gphQuadfz(pp,1:10)=gphQuadf(1:10,3)';
...
Basis.Edge.gphQuadex=gphQuadex;
Basis.Edge.gphQuadey=gphQuadey;
Basis.Edge.gphQuadez=gphQuadez;
...
Basis.Face.phH1f1=phH1f1;
Basis.Face.phH1f2=phH1f2;
Basis.Face.gphQuadfx=gphQuadfx;
Basis.Face.gphQuadfy=gphQuadfy;
Basis.Face.gphQuadfz=gphQuadfz;
