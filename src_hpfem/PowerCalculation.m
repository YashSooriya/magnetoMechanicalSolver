function [OutPower] = PowerCalculation(Mesh,Unknown,Basis,Quadrature,...
    sol,ProblemData,freq,CondFactorBodies)
            ...
            for pp=1:nip
                ...
                for nBody=1:NmechBodies
                    if any(material==matMech(nBody,:))
                        OutPower(nBody)=OutPower(nBody)+0.5*omega^2*CondFactorBodies(nBody)*sigma(material)*abs((e'*e))*intw(pp)*det;
                    end
                end
