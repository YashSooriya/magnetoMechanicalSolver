function [OutVectorPotential] = CurlNormACalculationStaggered(Mesh,Unknown,Basis,Quadrature,...
...
OutNormCurlA=zeros(1,NmechBodies);
            ...
            for pp=1:nip
                ...
                for nBody=1:NmechBodies
                    if any(material==matMech(nBody,:))
                        OutNormCurlA(nBody)=OutNormCurlA(nBody)+abs((curlE'*curlE))*intw(pp)*det;
                    end
                end
...
% magnitude 
OutNormCurlA = sqrt(OutNormCurlA)
disp(['The Output Norm of curlA is: ', num2str(OutNormCurlA(1))]);