function [OutPower,OutPower4K,OutPower77K,OutPowerOVC] = PotentialCalculationStaggered(Mesh,Unknown,Basis,Quadrature,...
    sol,ProblemData,freq,UnknownStatic,solStatic)
       ...
       if gorder>1
        ...
        if flag==0
            ...
            for pp=1:nip
                ...
                ElectricField=1i*omega*(-e);
                
                % The output power is:
                
                OutPower=OutPower+abs((ElectricField'*ElectricField))*intw(pp)*det;
                if any(material==mat4K)
                    OutPower4K=OutPower4K+abs((ElectricField'*ElectricField))*intw(pp)*det;
                elseif any(material==mat77K)
                    OutPower77K=OutPower77K+abs((ElectricField'*ElectricField))*intw(pp)*det;
                elseif any(material==matOVC)
                    OutPowerOVC=OutPowerOVC+abs((ElectricField'*ElectricField))*intw(pp)*det;
                end
...
disp(['The Output Power is: ', num2str(OutPower)]);