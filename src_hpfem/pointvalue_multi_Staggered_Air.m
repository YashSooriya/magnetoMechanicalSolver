...
function pointvalue_multi_Staggered_Air(mesh,unknown,ProblemData,Solution,probstatic,freq)
...
for imat = matAir:matAir
        ...
        if flag==0
            ...
            for j = 1:nppt
                ...
                gph=[gphxstore(:,j) gphystore(:,j) gphzstore(:,j)];
                   ...
                   if gorder>1 
                    [~,~,~,asxi,aseta,aszeta,~]=jacobian_pre(flag,gesizet,gph,mycoord) ;
                   else
                    gphQuad=[gphxstoreQuad(:,j) gphystoreQuad(:,j) gphzstoreQuad(:,j)];
                                
                    % evaluate covairant mapping
                    [~,~,~,asxi,aseta,aszeta,~]=jacobian_pre(flag,gesizet,gphQuad,mycoord);
                   end
    ...
    if probstatic==0
        filename=[job '_StaggeredSolver2Tol7DeformedAir' num2str(imat-1) '_' num2str(freq) 'Hz' '_p' num2str(orderH1) '_q' num2str(order) '.vtu']
        vtuk_puvw_write_AirLight ( filename, npoinn, nelemn, ...
            coordn, intman,phen)
        disp('done')
    else
