function [OutCurlNormA4K,OutCurlNormA77K,OutCurlNormAOVC] = CurlNormACalculationStaggeredMRI(Mesh,Unknown,Basis,Quadrature,...
        ...
        if flag==0
            ...
            for pp=1:nip
                ...
                if any(material==mat4K)
                    OutCurlNormA4K=OutCurlNormA4K + abs((curlE'*curlE))*intw(pp)*det;
                elseif any(material==mat77K)
                    OutCurlNormA77K=OutCurlNormA77K + abs((curlE'*curlE))*intw(pp)*det;
                elseif any(material==matOVC)
                    OutCurlNormAOVC=OutCurlNormAOVC + abs((curlE'*curlE))*intw(pp)*det;
                end
...
OutCurlNormA4K = sqrt(OutCurlNormA4K);
OutCurlNormA77K = sqrt(OutCurlNormA77K);
OutCurlNormAOVC = sqrt(OutCurlNormAOVC);
disp(['The Output Norm Curl A is: ', num2str(OutCurlNormA4K)]);