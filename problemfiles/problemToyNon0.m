function probdata = problemToyNon0(pm)
...
job=sprintf('ToyExtra1');       % Mesh name
...
nmat = 11;                            % Number of materials
muz = 1.256637061435917e-06;         % Mu_z (Free sopace permeability)
epz = 0;                             % Ep_z
jsrc=[0 0 0 0 0 0];
% Conductivity, gamma
regTerm=1e-4;
sigma   = [regTerm regTerm regTerm regTerm 71e6 71e6 33e6 33e6 1.4e6 1.4e6 regTerm regTerm];
%sigma   = [regTerm regTerm regTerm regTerm 3.5e6 3.5e6 1.65e6 1.65e6 1.4e6 1.4e6 regTerm regTerm regTerm];

% Density, rho
rho     = [0 0 0 0 2710 2710 2698 2698 7900 7900 0 0];

% Young's Modulus, E
E       = [0 0 0 0 81 81 81 81 210 210 0 0]*1e9;
% Mu_r, Ep_r, Sigma, J

% Poisson's Ratio, nu
nu      = [0 0 0 0 0.338 0.338 0.337 0.337 0.283 0.283 0 0];
...
% probdata.non0=[x_dir, y_dir, z_dir];
...
function e=esproblemdir(x,y,z,index,arg,probstatic,probFlag,omega,x_dir,y_dir,z_dir)    
