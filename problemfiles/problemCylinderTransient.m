function probdata=problemCylinderTransient(pm,option,Jrat)
...
elseif probFlag==2                      % Mechanical problem
  omega=arg.omega;
    % Output vector
    e=[x^4*cos(omega)+y^4*sin(omega)+z^4;0;0];
end
...
grade=zeros(3,3);
grade(1,1)=4*x^3*cos(omega);
grade(1,2)=4*y^3*sin(omega);
grade(1,3)=4*z^3;
grade(2,1)=0;
grade(2,2)=0;
grade(2,3)=0;
...
rho=arg.rho;
omega=arg.omega;
%srcmech=[12*x^2*(lambda+2*G)+12*G*y^2+12*G*z^2+rho*omega^2*(x^4+y^4+z^4);0;0];
srcmech=[12*x^2*cos(omega)*(lambda+2*G)+12*G*y^2*sin(omega)+12*G*z^2+rho*omega^2*(x^4*cos(omega)+y^4*sin(omega)+z^4);0;0];
...
elseif probFlag==2                  % Mechanical problem
  ...
  omega=arg.omega;
    % Output vector
    out=[x^4*cos(omega)+y^4*sin(omega)+z^4;0;0];
end
